      ...
      >
        ...
        <View style={tmStyles.progressCard}>
          ...
          <View style={tmStyles.progressTrack}>
            <View
              style={[tmStyles.progressFill, { width: `${progressPercent}%` }]}
            />
          </View>
        </View>

        <ScrollView
          contentContainerStyle={tmStyles.grid}
            ...
            {BUILDINGS.map((b, i) => {
              ...
              return (
                <BuildingCard
                  ...
                  onPress={() => {
                    if (!locked)
                      navigation.navigate('Interior', { building: b });
                  }}
                />
              );
            })}
...
function BuildingCard({ building: b, locked, index, onPress }) {
  return (
    ...
    >
      <LinearGradient
        colors={
          locked ? ['#E8E0D0', '#D8D0C0'] : [b.bgColor || '#FFF8F0', '#FFFFFF']
        }
  ...
  const scenes = {
    cafe: () => (
      <G>
        ...
        <Polygon
          points={`0,${H * 0.6} ${W},${H * 0.6} ${W},${H} 0,${H}`}
          fill='#D4A870'
        />
        <Rect
          x={W * 0.55}
      ...
      <G>
        ...
        {Array.from({ length: 12 }, (_, i) => (
          <Circle
            key={i}
            cx={i * (W / 12) + 8}
            cy={H * 0.08 + Math.sin(i) * H * 0.05}
            r={2}
            fill={`hsl(${40 + i * 20},100%,75%)`}
          />
        ))}
    ...
    >
      ...
      {locked && (
        <G>
          ...
          <Path
            d={`M${W / 2 - 7},${H / 2 - 2} L${W / 2 - 7},${H / 2 + 8} L${W / 2 + 7},${H / 2 + 8} L${W / 2 + 7},${H / 2 - 2} Z`}
            fill='white'
          />
          <Path
            d={`M${W / 2 - 5},${H / 2 - 2} Q${W / 2 - 5},${H / 2 - 12} ${W / 2},${H / 2 - 12} Q${W / 2 + 5},${H / 2 - 12} ${W / 2 + 5},${H / 2 - 2}`}
            stroke='white'
            strokeWidth={3}
            fill='none'
          />
